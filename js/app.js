...
let keyboardViewportLockUntil = 0;
...
function updateStableViewportHeight(allowShrink = false) {
  const nextHeight = Math.round(window.innerHeight || document.documentElement?.clientHeight || 0);
  if (!nextHeight) return;

  const shrinkLocked = isTextInputFocused() || Date.now() < keyboardViewportLockUntil;
  if (!stableViewportHeight || nextHeight > stableViewportHeight || ((allowShrink || !shrinkLocked) && !isTextInputFocused())) {
    stableViewportHeight = nextHeight;
    document.documentElement.style.setProperty("--stable-viewport-height", `${stableViewportHeight}px`);
  }
}
...
function init() {
  ...
  window.visualViewport?.addEventListener("resize", scheduleViewportChangeWithRetries);
  window.visualViewport?.addEventListener("scroll", scheduleViewportChange);
  document.addEventListener("focusin", handleViewportFocusChange);
  document.addEventListener("focusout", handleViewportFocusChange);
...
function scheduleViewportChangeWithRetries() {
  scheduleViewportChange();
  [120, 320, 640].forEach((delay) => {
    setTimeout(scheduleViewportChange, delay);
  });
}
...
function handleViewportFocusChange(event) {
  const target = event.target;
  if (!(target instanceof HTMLInputElement || target instanceof HTMLTextAreaElement || target instanceof HTMLSelectElement)) {
    return;
  }

  keyboardViewportLockUntil = Date.now() + 900;
  scheduleViewportChangeWithRetries();
}
