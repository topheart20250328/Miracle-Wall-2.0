...
function getViewportSize() {
  const stableHeight = parseFloat(getComputedStyle(document.documentElement).getPropertyValue("--stable-viewport-height"));
  const width = Math.round(window.innerWidth || document.documentElement?.clientWidth || viewportSize.width || 0);
  const height = Math.round(stableHeight || window.innerHeight || document.documentElement?.clientHeight || viewportSize.height || 0);
...
export function syncViewportSize() {
  ...
  if (!app?.renderer || !app.view || !viewportSize.width || !viewportSize.height) {
    return;
  }

  app.renderer.resize(viewportSize.width, viewportSize.height);
  app.view.style.width = `${viewportSize.width}px`;
  app.view.style.height = `${viewportSize.height}px`;
}

// Mock other exports to satisfy app.js imports
export function resetStickerScale() {}

export function getStickerRect(node, id) {
  const targetId = id || (node && node.dataset && node.dataset.id);
  const sprite = spriteMap.get(targetId);
  if (!sprite) return null;

  // Ensure sprite world transform is up to date
  // sprite.getBounds() calculates it in global space (screen coords due to full-screen canvas)
  const bounds = sprite.getBounds();
  
  if (!bounds || (bounds.width === 0 && bounds.height === 0)) {
     // Fallback: calculate manually using position and mainContainer scale
     const globalPos = sprite.getGlobalPosition();
     const scale = mainContainer ? mainContainer.scale.x : 1;
     const size = 60 * scale; // Approx visual size
     return {
         left: globalPos.x - size/2,
         top: globalPos.y - size/2,
         width: size,
         height: size
     };
  }
    ...
    if (wallSvg && globalViewBox) {
      ...
      if (wrapper) {
          ...
          if (svgWidth > 0 && svgHeight > 0) {
              ...
              const screenRight = viewportSize.width + buffer;
              const screenBottom = viewportSize.height + buffer;
